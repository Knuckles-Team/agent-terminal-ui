        ...
        if file_path.parent == repo_path:
            ...
            if file_path.suffix == ".txt":
                if file_path.name.lower() not in ALLOWED_TXT_NAMES:
                    violations.append(
                        f"Non-standard root-level text file detected: "
                        f"'{file_path.name}'. Only 'requirements.txt' and "
                        f"'requirements-dev.txt' are allowed."
                    )
                ...
                for label, pattern in SECRET_PATTERNS:
                    for match in pattern.findall(line):
                        ...
                        if not is_placeholder(match_str):
                            ...
                            violations.append(
                                f"Potential unmasked secret ({label}) detected "
                                f"in {rel_path}:{idx}\n"
                                f"  Line: {line.strip()}"
                            )
        except Exception:
...
def main():
    ...
    if violations:
        ...
        for idx, violation in enumerate(violations, 1):
            print(f"\n[{idx}] {violation}")
        print(
            "\nNote: To bypass secret checks on specific lines, append "
            "'# sanitizer:ignore' to the end of the line."
        )
        sys.exit(1)
