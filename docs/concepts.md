...
|------------|------|-------------|
| `CONCEPT:TU-OS.governance.tui` | Core API Client | Primary API client for Terminal UI for Agentic AI! |
| `CONCEPT:TU-OS.governance.tui-2` | MCP Server | Model Context Protocol server entry point |
| `CONCEPT:TU-OS.governance.tui-3` | A2A Agent | Agent-to-Agent protocol server |
...
This project integrates with `agent-utilities` via `CONCEPT:AU-ECO.messaging.native-backend-abstraction` (Unified Toolkit Ingestion). The `agent_terminal_ui` MCP server registers its tools with the agent-utilities FastMCP middleware, enabling automatic discovery, telemetry, and Knowledge Graph ingestion of all TUI-* concepts.
